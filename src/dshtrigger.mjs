...
// ---- live daemon health via /proc (Linux) ----
function daemonAlive() {
  const pats = ['dsh_bridge.mjs', 'bridge-daemon.mjs'];
  try {
    const procs = readdirSync('/proc').filter(n => /^\d+$/.test(n));
    for (const pid of procs) {
      try {
        const cmd = readFileSync('/proc/' + pid + '/cmdline', 'utf8').replace(/\0/g, ' ');
        for (const p of pats) if (cmd.includes(p)) return { running: true, pid: Number(pid), script: p, cmd: cmd.trim().slice(0, 90) };
      } catch {}
    }
  } catch {}
  return { running: false, hint: 'no daemon child in /proc; run "node src/dshtrigger.mjs daemon" (or rely on the container entrypoint)' };
}
...
async function triggerSend() {
  ...
  while (Date.now() - t0 < timeout) {
    if (existsSync(resultPath)) {
      ...
      if (NOTIFY && r && r.human && SENDS && CHANNEL) { try { execFileSync(process.execPath, [SENDS, CHANNEL, r.human], { timeout: 25000, encoding: 'utf8' }); } catch {} }
      return r;
    }
